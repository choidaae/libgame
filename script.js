...
function getInputResult(input) {
    const keyword = {
        ...
        '시작': '어둠에 눈이 조금 익숙해지고 나서야 내가 도서관에 들어와있다는 걸 알았다. 쉬는시간에 잠깐 잠든 것 뿐인데 내가 왜 여기 있는거지?<br><br>일단 너무 어두워서 전등 스위치를 켜야겠다.<br><br><br>🔔입력 키워드는 "지문 안"에 숨겨져 있습니다.<br>다음 장면으로 이동하는 키워드일 수도 있고, 여러분들이 탐색해야 하는 대상일 수도 있습니다.<br><br>힌트를 드리죠. 지문에서 너무 어두워 스위치를 켜야겠다고 하네요! <span class="pp">스위치</span>를 입력해보세요.',

        '스위치': '달칵- 달칵- 전등이 켜지지 않는다. 핸드폰 플래시를 켜려고 주머니에 손을 넣으니 구겨진 초대장이 손에 잡힌다.<br>핸드폰 배터리는 5%밖에 남지 않아 금방이라도 꺼질 것 같다. 눈 앞엔 사물함이 보인다.<br><br><br>🔔입력 키워드는 미스테리를 해결하기 위한 아이템일 수도 있습니다.<br><br>힌트를 드리죠. 주머니에 손을 넣어 잡힌<span class="pp">초대장</span>을 입력창에 입력해보세요.<br><br>🔔그리고 지문에서 다음 장면으로 가기 위한 <span class="pp">?키워드?</span>를 찾아보세요.',
